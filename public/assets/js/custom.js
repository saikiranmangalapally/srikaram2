(function ($) {
  ...
  function handlePreloader() {
    if ($('.loader-wrap').length) {
      $('.loader-wrap').delay(500).fadeOut(500, function() {
        $(this).remove();
        // Check for elements that should be revealed immediately on load
        checkReveals();
      });
    }
  }

  // Srikaram Reveal Logic
  ...
  function initReveals() {
    document.querySelectorAll('.sr-reveal, .sr-reveal-left, .sr-reveal-right').forEach(el => {
      revealObserver.observe(el);
    });
  }

  function checkReveals() {
    // Manually reveal elements that are already in the viewport
    document.querySelectorAll('.sr-reveal, .sr-reveal-left, .sr-reveal-right').forEach(el => {
  ...
  $(window).on('load', function() {
    handlePreloader();
    initReveals();
    // Final safety check after all assets are loaded
    setTimeout(checkReveals, 1000);
  });

  $(window).on('scroll', function() {
    // Throttled scroll check for reveals (secondary safety)
